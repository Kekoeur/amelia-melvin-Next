...
export type Step = "input" | Moment | "Allergies" | "Allergies2" | "Message" | "Recap";

export default function RepondreForm({ invites, allergenes }: Props) {
  ...
  const steps = useMemo<Step[]>(() => {
    if (!selectedInvite) return ["input"];
    const moments = getMomentsFromQuand(selectedInvite.Quand);
    return ["input", ...moments, "Allergies", "Allergies2", "Message", "Recap"];
  }, [selectedInvite]);
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto">
      ...
      <div className="space-y-4">
        {currentStep === "input" && (
          <div>
            <label className="block mb-2">Entrez votre nom ou prénom :</label>
            <input
        ...
        {selectedInvite && ["Matin", "Midi", "Soir", "Retour"].includes(currentStep) && (
          <div>
            <p className="font-bold mb-2">{currentStep} - Qui sera présent ?</p>
